...
direction_t is_my_direction(int key, struct Snake* my_snake){
    keys_t* key_ptr = my_snake->controll_keys;
    while((key_ptr->key) != 0){
        if(key == key_ptr->key){
...
static uint8_t is_opposite_direction(direction_t my_dir, direction_t new_dir){
    switch(my_dir){
        case DIR_UP:    if(new_dir == DIR_DOWN)  return 1;
        case DIR_DOWN:  if(new_dir == DIR_UP)    return 1;
        case DIR_LEFT:  if(new_dir == DIR_RIGHT) return 1;
        case DIR_RIGHT: if(new_dir == DIR_LEFT)  return 1;
        default: return 0;
    }
    return 0;
}
