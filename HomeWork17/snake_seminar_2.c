...
//Opposite direction controll, ignored direction key if it cause change direction in opposite side
uint8_t isOppositeDirection(int key, int cur_direction){
    switch(key){
        case 'w': case 'W': case 1094: case 1062: if(cur_direction == DOWN)  return 1;  break;
        case 'a': case 'A': case 1092: case 1060: if(cur_direction == RIGHT) return 1;  break;
        case 'd': case 'D': case 1074: case 1042: if(cur_direction == LEFT)  return 1;  break;
        case 's': case 'S': case 1099: case 1067: if(cur_direction == UP)    return 1;  break;
        case KEY_UP:                              if(cur_direction == DOWN)  return 1;  break;
        case KEY_DOWN:                            if(cur_direction == UP)    return 1;  break;
        case KEY_RIGHT:                           if(cur_direction == LEFT)  return 1;  break;
        case KEY_LEFT:                            if(cur_direction == RIGHT) return 1;  break;
    }
    return 0;
}
...
void changeAllDirections(snake_t snakes[], size_t num_snakes, const int32_t key) {
    for (size_t i = 0; i < num_snakes; i++) {
        ...
        if (i == 1) {
            if(!isOppositeDirection(key, snakes[i].direction)){
            ...
            if(!isOppositeDirection(key, snakes[i].direction)){
                ...
                switch(key){
                    case KEY_UP:    snakes[i].direction = UP; break;
                    case KEY_DOWN:  snakes[i].direction = DOWN; break;
                    case KEY_LEFT:  snakes[i].direction = LEFT; break;
                    case KEY_RIGHT: snakes[i].direction = RIGHT; break;
                    default: break;
                }
            }
        }
...
void playGame() {
    ...
    while (key_pressed != STOP_GAME && !game_over && game_running) {
        ...
        static int stored_score_1 = 1, stored_score_2 = 1; 
        ...
        //To avoid flicker need to update screen only if scores changed
        if((stored_score_1 != snakes[0].score) || (stored_score_2 != snakes[1].score)){
            stored_score_1 = snakes[0].score;
            stored_score_2 = snakes[1].score;
            clear();
        }
