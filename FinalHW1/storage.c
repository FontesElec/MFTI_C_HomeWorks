...
ProcResult push_to_storage(ProtectedStorage* storage, TempData data){
    /*
        ...
        2. Смотрим, если новая дата позднее последней, или раньше первой
        3. Проходим по узлам хранилища, пока не упрёмся в null или пока не найдём дату, старше вносимой
        4. Если список был пуст, то вставляем новую запись, указатели на начало и конец списка указывают на новую запись
        5. Если список имеет записи, то проходим по списку в поиске либо конца, либо даты, старше новой, вставляем новый узел
    */
    ...
    //Проверка на выход за параметр
    if(data.day > 31 || data.month > 12 || data.hour > 23 || data.min > 59 ){
        printf("Error data format, abort adding new data\n");
        return FAIL;
    }
    //Копируем информацию
    new_node->data = data;
    ...
    //проверяем, старше ли текущая дата относительно последней в списке
    if(is_date_bigger(&(storage->tail->data), &data)){
    ...
    //Если новая запись раньше первой из списка
    if(is_date_bigger(&data, &(storage->head->data))){
        //Если узел первый в списке, то вставка в начало
        //запоминаем старый первый узел
        current_node = storage->head;
